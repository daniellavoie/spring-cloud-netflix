...
public class HystrixSecurityAutoConfiguration {
	...
	private CustomCommandExecutionHook customCommandExecutionHook;
	...
	public void init() {
		...
		// Registers existing plugins excepts the Command Hook.
		HystrixPlugins.getInstance().registerConcurrencyStrategy(concurrencyStrategy);
		HystrixPlugins.getInstance().registerEventNotifier(eventNotifier);
		HystrixPlugins.getInstance().registerMetricsPublisher(metricsPublisher);
		HystrixPlugins.getInstance().registerPropertiesStrategy(propertiesStrategy);
		HystrixPlugins.getInstance().registerCommandExecutionHook(
				new SecurityContextRegistratorCommandHook(customCommandExecutionHook));
	}

	@Bean
	public HystrixRequestContextFilter hystrixRequestContextEnablerFilter() {
